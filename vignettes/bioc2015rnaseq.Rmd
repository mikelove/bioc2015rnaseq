...
The following line creates the *TxDb*.
...
See the `r Biocpkg("GenomicFeatures")` packge for more details.
...
package. Here we show an example of counting, using only the
...
```{r}
library("GenomicAlignments")
total.range <- range(unlist(range(ebg)), ignore.strand=TRUE)
...
cov <- coverage(ga)
cov[total.range * 2]
cov.numeric <- as.numeric(cov[total.range * 2][[1]])
plot(cov.numeric, cex=.1)
```
